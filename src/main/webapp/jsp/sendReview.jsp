...
<body>
	...
	try {
		...
		Float food_quality = Float.parseFloat(request.getParameter("food_quality")); 
		Float service = Float.parseFloat(request.getParameter("service")); 
		Float location = Float.parseFloat(request.getParameter("location")); 
		...
		float avgRate = (food_quality+service+location+price)/4;
...
// 		out.print(username+"<br>");
		...
		String insert = "INSERT INTO `demo`.`restaurantRate` (`restaurantName`, `rate`, `username`, `title`, `content`, `postTime`)" 
		+ "VALUES (?, ?, ?, ?, ?, ?)";

		PreparedStatement ps = con.prepareStatement(insert);
		
		ps.setString(1,resturantName);
		ps.setFloat(2,avgRate);
		...
		String update = "UPDATE `demo`.`restaurant` SET `rate` = (SELECT avg(rate) FROM demo.restaurantRate where restaurantName='"+resturantName+"') WHERE (`restaurantName` = '"+resturantName+"')";
	...
	<div class="payment-success-popup page--popup" id="payment-success">
	  ...
	  <p class="popup--subtitle">Your Review is posted</p>
